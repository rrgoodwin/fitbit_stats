...
@filepath = "#{@root_filepath}/data/heart_rate_json/"
...
def write_file_for_date(date)
  ...
  hrs = @client.intraday_heartrate_time_series(
    start_date: date,
    end_date: date,
    detail_level: "1min",
...
def get_data!(force=false)
  date_range = if force
    ...
    latest_date = DateTime.parse(Dir["#{@filepath}/*"].max.scan(/\d{6}/).first) rescue @start_date
